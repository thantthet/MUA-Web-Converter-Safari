 ...
 *   It use Parabaik Converter developed by Ko Ngwe Htun
...
var disabledByPage = document.getElementById("disableMUA") ? true : false;
console.log("MUA Conver is disabled by site was " + disabledByPage);

function onDocumentReady() {
    safari.extension.dispatchMessage("DocumentIsReady");
}

function convertAndObserve() {
    // convert the site
    convertTree(document.body);
    // listen for further dom changes
    addObserver();
}
...
safari.self.addEventListener("message", function handleMessage(event) {
    ...
    if (event.name == "ConvertAndObserve") {
    	convertAndObserve();
    }
});
...
if (!disabledByPage) {
    ...
    // 2. document is ready for conversion
    else {
        onDocumentReady();
    }
}
